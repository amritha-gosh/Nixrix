/**
 * NIXRIX shared helpers — src/app/pages/_nixrix-helpers.tsx
...
// ── Progressive backgrounds — lightest at top, darkens per section ─────────
export const BG = {
  hero: "linear-gradient(160deg,#FAF5EC 0%,#F5EDE0 100%)", // always hero
  s1:   "#F5EDE0",
  s2:   "#EDE4D4",
  s3:   "#E5D9C6",
  dark: "#1A1208",
...
// ── Section tag ────────────────────────────────────────────────────────────
export function Tag({
  children, onDark = false, center = true,
}: { children: ReactNode; onDark?: boolean; center?: boolean }) {
  return (
...
export const ALL_REVIEWS = [
  ...
  { name:"Marcus Reid",     role:"Managing Director",  company:"Reid and Co Accountants, Manchester", avatar:"https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=120&h=120&fit=crop&crop=face", text:"I was spending two hours a day on things that should have been automatic. The Autopilot package changed that completely. I do not know how we managed without it.", stars:5 },
  { name:"Priya Sharma",    role:"Operations Manager", company:"Greenfield Lettings, Birmingham",     avatar:"https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=120&h=120&fit=crop&crop=face", text:"Our old website was doing nothing. After NIXRIX rebuilt it and connected it to our CRM we went from three web enquiries a month to fourteen.", stars:5 },
  ...
  { name:"Daniel Clarke",   role:"Founder",            company:"Clarke Recruitment, Leeds",           avatar:"https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=120&h=120&fit=crop&crop=face", text:"Responsive, honest, and delivered ahead of schedule. I have worked with bigger agencies that took three times as long and communicated half as well.", stars:5 },
...
export function ReviewRibbon({ bg = "#EDE4D4" }: { bg?: string }) {
  ...
  return (
    <div className="relative overflow-hidden py-4" style={{ background: bg }}>
      ...
      >
        {doubled.map((r, i) => (
          <div key={i} className="w-[310px] shrink-0 rounded-2xl border border-[#1A1208]/10 bg-white p-5 shadow-[0_4px_16px_rgba(26,18,8,0.06)]">
            <Stars n={r.stars} />
            <p className="mt-3 text-sm font-light leading-[1.8] text-[#1A1208]/70"
              style={{ fontFamily: F.b }}>"{r.text}"</p>
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        background: "radial-gradient(circle,rgba(232,35,10,0.13) 0%,transparent 68%)",
      }}
